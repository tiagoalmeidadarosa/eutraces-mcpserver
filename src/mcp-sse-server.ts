    ...
    app.post('/mcp', express.json(), async (req, res) => {
      try {
        ...
        switch (method) {
          ...
          case 'resources/list':
            // Return resources list similar to the main server
            const resources = [];
          ...
          case 'tools/call':
            // Handle tool calls - this is the main implementation
            ...
            switch (toolName) {
              case "query_endpoint": {
                ...
                if (endpoints.length === 0) {
                  ...
                  const endpointResult = endpoints.map(e => ({
                    name: e.name,
                    method: e.method,
                    url: e.url,
                    description: e.description,
                  ...
                  result = {
                    content: [{
                      type: "text",
                      text: JSON.stringify(endpointResult, null, 2)
                    }]
                  };
                }
                break;
              ...
              case "get_examples": {
                ...
                if (examples.length === 0) {
                  ...
                  const examplesResult = examples.map(e => ({
                    name: e.name,
                    type: e.type,
                    operation: e.operation,
                    content: e.content,
                  ...
                  result = {
                    content: [{
                      type: "text",
                      text: JSON.stringify(examplesResult, null, 2)
                    }]
                  };
                }
                break;
